...
if __name__ == '__main__':
    ...
    weight_path = Path(__file__).parent / "uni/pytorch_model.bin"
    model.load_state_dict(torch.load(os.path.join(weight_path), map_location="cpu"), strict=True)
    ...
    for bag_candidate_idx in range(total):
        slide_filename = bags_dataset[bag_candidate_idx]
        slide_id = slide_filename.split(args.slide_ext)[0]
