...
class ABMIL_Surv_Img(nn.Module):
    def __init__(self, feat_type='uni', clinical_dim=27, rna_input_dim=19359):
        super().__init__()
        if feat_type == 'uni':
            size = [1024, 512, 256]
    ...
    def forward(self, h, clinic, rna, attention_only=False):
        ...
        return logits.squeeze(1), A

class ABMIL_Surv_ImgCli(nn.Module):
    def __init__(self, feat_type='uni', clinical_dim=27, rank=64):
        super().__init__()
        if feat_type == 'uni':
            size = [1024, 512, 256]
    ...
    def forward(self, h, clinic, rna, attention_only=False):
        ...
        M = torch.mm(A, h_feat)
        hc = self.fuse_hc(M, clinic)
        logits = self.classifier(hc)
        return logits.squeeze(1), A


class ABMIL_Surv_ImgRNA(nn.Module):
    def __init__(self, feat_type='uni', rna_input_dim=19359, rank=64):
        super().__init__()
        if feat_type == 'uni':
            size = [1024, 512, 256]
    ...
    def forward(self, h, clinic, rna, attention_only=False):
        ...
        rna_feat = self.rna_encoder(rna)
        hr = self.fuse_hr(M, rna_feat)
        logits = self.classifier(hr)
        return logits.squeeze(1), A

class ABMIL_Surv_RNACli(nn.Module):
    def __init__(self, clinical_dim=27, rna_input_dim=19359, rank=64, feat_type='uni'):
        super().__init__()
        self.rna_encoder = RNAEncoder(input_dim=rna_input_dim, output_dim=128)
